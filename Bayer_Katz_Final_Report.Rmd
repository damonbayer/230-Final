...
In this paper, we implement data augmentation Gibbs sampling for probit regression, multinomial probit of ordered data, and an extension of the probit model, the t-link model which were introduced by Albert and Chib (1993). In this section we will detail the methods and provide the algorithms for each. 
...
The T-link is an extension of the above model, which allows use to use the t-distribution as our link function instead of the normal distribution. By generalizing the model, we can now choose a distribution that better fits out model. Usually, we are concerned with logistic regression because of the interpretability of this particular model. The flexibility of the t-link function allows us to draw from a posterior distribution of $\vec \beta$ that is approximately the posterior distribution if we had chosen to use the logit link function. According to Albert and Chib, a t-distribution with a degrees of freedom of 8 is a fairly close approximation of the logistic regression model.

To implement this link function, we must introduce a second set of latent variables, $\lambda_i$.The details of this method can be found in the algortihm.  
Description:
