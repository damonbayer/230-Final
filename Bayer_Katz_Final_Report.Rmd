...
In this paper, we implement data-augmentated Gibbs sampling for probit regression on binary response data, multinomial probit regression on ordinal response data, and an extension of the probit regtession model, the t-link regression model for binary response data [@albertchib93]. These methods are used to generate samples from posterior distributions for Bayesain inference on regression coefficients. In this section, we detail each method, including pseudocode for its algorithm.
